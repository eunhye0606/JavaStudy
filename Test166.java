...
○ Enumeration 과 Iterator 인터페이스
   ...
   - java.util.Itetator<E> 인터페이스
     :Enumeratoin 과의 차이점은 단지 『remove()』메소드가 추가된 것 뿐이다.
	  Iterator 가 가지고 있는 『hasNext()』와 『next()』 메소드는
	  이름만 약간 다를 뿐 Enumeration 인터페이가 가지고 있는
	  『hasMoreElements()』와 『nextElement()』와
	  정확히 일치하는 기능을 수행한다.
	  Enumeratoin 대신 Iterator 를 JDK 1.2부터 추가하여 사용하게 된 이유는
	  Enumeratoin 인터페이스는 집합 내에서 요소를 제거할 방법이 없기 때문에
...
public class Test166
{
	public static void main(String[] args)
	{
		...
		//벡터의 elements() 메소드 :Enumeration 타입 반환
		...
		*/
		...
		//벡터의 iterator() 메소드 : Iterator 타입 반환
		while (it.hasNext())
		{
